...
<body>
  <div id="wrapper">
    ...
    <main id="main" class="sub_main cpList">
      <!-- 서브페이지 lnb -->
      <div class="lnb_wrap">
        <h2 class="lnb_title another">전체 캠페인</h2>
        <span class="lnb_bar"></span>
      </div>
      <div class="snb_wrap">
        <ul class="snb" style="margin-top:30px;">
        ...
        <ul class="campaign_list">
          ...
          <li>
            <div class="campaign <?if($row['CPT_APP_END_DATE'] < $date){ echo " end "; }?>">
              <a
                href="view.php?mode=CP_VIEW&CPT_IDX=<?=$row['CPT_IDX']?>&search=<?=$search?>&type=<?=$row['CPT_CATE2']?>&cate2=<?=$cate2?>&cate3=<?=$cate3?>&sort=<?=$sort?>&pageNo=<?=$pageNo?>">
