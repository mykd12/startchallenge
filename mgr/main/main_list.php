          ...
          <div class="g-hidden-sm-down g-bg-gray-light-v8 g-pa-20">
            <ul class="u-list-inline g-color-gray-dark-v6 add-location">
              ...
              <li class="list-inline-item">
                <span class="g-valign-middle">메인비쥬얼 리스트</span>
              </li>
            </ul>
          </div>
          <!-- End Breadcrumb-v1 -->
          <div class="g-pa-20 CTs-list-notice m---style01" id="CTs-list">
            ...
            <div class="media">
              <div class="d-flex align-self-center">
                <h1 class="g-font-weight-300 g-font-size-25 g-color-black mb-0 page-big-title">메인비쥬얼 리스트</h1>
              </div>
              <div class="media-body align-self-center text-right">
								<a class=" btn btn u-btn-primary g-width-100--md g-font-size-default g-ml-10" href="main_write.php?mode=MVINSERT&pageNo=<?=$pageNo?>&search=<?=$search?>">+ ADD</a>
              </div>
            </div>
            ...
            <div class="media-md align-items-center g-mb-30">
              <div class="media d-md-flex align-items-center ml-auto">
                <div class="d-flex g-ml-15 g-ml-55--md">
									<form action='main_list.php' method='get' id='frm' name='frm'>
									...
									<?while($row = mysqli_fetch_array($result)){ // 반복문?>
										<tr>
											...
											<td class='table_title'>
												<a href='main_write.php?mode=MVMODIFY&MVT_IDX=<?=$row['MVT_IDX']?>&pageNo=<?=$pageNo?>&search=<?=$search?>' class="g-color-black"><?=stripslashes($row['MVT_TITLE'])?></a>
											</td>
											<td><?=$row['MVT_REG_DATE']?></td>
											<td class="text-right" >
												<a href="main_write.php?mode=MVMODIFY&MVT_IDX=<?=$row['MVT_IDX']?>&pageNo=<?=$pageNo?>&search=<?=$search?>" class="btn btn-xs u-btn-indigo g-mr-10 ">
													<i class="hs-admin-pencil g-mr-3"></i> MODIFY
												</a>
												<a href="../inc/adminDAO.php?mode=MVDELETE&MVT_IDX=<?=$row['MVT_IDX']?>&pageNo=<?=$pageNo?>&search=<?=$search?>" class="btn btn-xs u-btn-primary g-mr-10 " onclick="return confirm('삭제하시겠습니까?');">
													<i class="hs-admin-trash g-mr-3"></i> DELETE
												</a>
											</td>
										</tr>
									<?$i++;}?>
