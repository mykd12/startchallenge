...
<body>
  <div id="wrapper">
    ...
    <main id="main" class="sub_main cpList">
      <!-- 서브페이지 lnb -->
      <div class="lnb_wrap">
        <h2 class="lnb_title another">제품 캠페인</h2>
        <ul class="lnb">
          <li <?if($cate2=='ALL' || !$cate2){ echo " class='on' " ; }?>><a
              href="./<?=$page?>?search=<?=$search?>&cate2=ALL&cate3=<?=$cate3?>&sort=<?=$sort?>">전체</a></li>
          <li <?if($cate2=='뷰티' ){ echo " class='on' " ; }?>><a
              href="./<?=$page?>?search=<?=$search?>&cate2=뷰티&cate3=<?=$cate3?>&sort=<?=$sort?>">뷰티</a></li>
          <li <?if($cate2=='생활리빙' ){ echo " class='on' " ; }?>><a
              href="./<?=$page?>?search=<?=$search?>&cate2=생활리빙&cate3=<?=$cate3?>&sort=<?=$sort?>">생활리빙</a></li>
          <li <?if($cate2=='디지털가전' ){ echo " class='on' " ; }?>><a
              href="./<?=$page?>?search=<?=$search?>&cate2=디지털가전&cate3=<?=$cate3?>&sort=<?=$sort?>">디지털가전</a></li>
          <li <?if($cate2=='교육도서' ){ echo " class='on' " ; }?>><a
              href="./<?=$page?>?search=<?=$search?>&cate2=교육도서&cate3=<?=$cate3?>&sort=<?=$sort?>">교육도서</a></li>
          <li <?if($cate2=='식품' ){ echo " class='on' " ; }?>><a
              href="./<?=$page?>?search=<?=$search?>&cate2=식품&cate3=<?=$cate3?>&sort=<?=$sort?>">식품</a></li>
          <li <?if($cate2=='기타' ){ echo " class='on' " ; }?>><a
              href="./<?=$page?>?search=<?=$search?>&cate2=기타&cate3=<?=$cate3?>&sort=<?=$sort?>">기타</a></li>
        </ul>
        <span class="lnb_bar"></span>
      </div>
      <div class="snb_wrap">
        <ul class="snb">
          <li class="pc_sns <?if($cate3=='전체' || !$cate3){ echo "on"; }?>"><a href="./<?=$page?>?search=<?=$search?>&cate2=<?=$cate2?>&cate3=전체&sort=<?=$sort?>"
              class="btn_sns_all">All</a></li>
          <li class="pc_sns <?if($cate3=='블로그'){ echo "on"; }?>"><a href="./<?=$page?>?search=<?=$search?>&cate2=<?=$cate2?>&cate3=블로그&sort=<?=$sort?>"
              class="btn_blog">Blog</a></li>
          <li class="pc_sns <?if($cate3=='인스타그램'){ echo "on"; }?>"><a href="./<?=$page?>?search=<?=$search?>&cate2=<?=$cate2?>&cate3=인스타그램&sort=<?=$sort?>"
              class="btn_insta">Instagram</a></li>
          <li class="pc_sns <?if($cate3=='유튜브'){ echo "on"; }?>"><a href="./<?=$page?>?search=<?=$search?>&cate2=<?=$cate2?>&cate3=유튜브&sort=<?=$sort?>"
...
$(".sort").click(function() {
  ...
  var search = "<?=$search?>";
  var page = "<?=$page?>";
  location.href = page + "?search=" + search + "&cate2=" + cate2 + "&cate3=" + cate3 + "&sort=" + sort;
});
//
-->
</script>
